...
import { FileText, BarChart3, History, Settings, WifiOff, RefreshCw, Loader2 } from "lucide-react";
...
function LoadingScreen() {
  return (
    <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
      ...
      <p className="text-sm text-[#64748B]">Cargando datos desde Google Sheets…</p>
    </div>
  );
}

export function Dashboard() {
  const { loading, syncing, offlineMode } = useStore();
  ...
  return (
    <div className="min-h-screen bg-[#0C112E] px-4 py-6 md:px-8 md:py-8">
      ...
      {/* Status bar */}
      <div className="mx-auto mb-3 max-w-6xl flex items-center justify-between gap-2 min-h-[24px]">
        {offlineMode && (
          <div className="flex items-center gap-1.5 rounded-full border border-[#F59E0B]/30 bg-[#F59E0B]/10 px-3 py-1 text-xs text-[#F59E0B]">
            ...
            Modo offline — datos locales (configura NEXT_PUBLIC_SHEETS_API_URL para sincronizar)
          </div>
        )}
        {syncing && !offlineMode && (
