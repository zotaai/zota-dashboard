  ...
  return (
    <div>
      ...
      <div className="mb-5 rounded-lg border border-black/[0.08] bg-black/[0.03] p-4">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            <User className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#94A3B8]" />
            <Input
      ...
      <div className="overflow-hidden rounded-lg border border-black/[0.08]">
        <div className="grid grid-cols-[1fr_50px] gap-2 border-b border-black/[0.08] bg-black/[0.03] px-4 py-2.5">
        ...
        <div className="divide-y divide-black/[0.06]">
          {state.users.map((user) => (
            <div
              key={user.id}
              className="grid grid-cols-[1fr_50px] items-center gap-2 px-4 py-2 transition-colors hover:bg-black/[0.03]"
            >
              <Input
                value={user.name}
                onChange={(e) => handleUpdate(user.id, e.target.value)}
                className="h-8 border-transparent bg-black/[0.04] text-sm text-[#1E293B] focus:border-[#0296DF] focus:bg-black/[0.06]"
              />
